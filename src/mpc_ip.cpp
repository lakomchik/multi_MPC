...
namespace MPC {
  ...
  //Auxilary function to add NLopt methods in class
  double customMinFunc(const std::vector < double > & x, std::vector < double > & grad, void * data) {
    MPC * c = (MPC * ) data;
    return c -> myfunc(x, grad);
  }

  void customEqConstraint(unsigned m, double * result, unsigned n,
  ...
  void MPC::euler_integration(Eigen::Matrix < autodiff::real, n_states, 1 >
    ...
    //Nonlinear next state calculating
  ...
  void MPC::set_init_cond(Eigen::Matrix < autodiff::real, n_states, 1 >
    const & init_cond) {
  ...
  void MPC::get_jacobian(Eigen::Matrix < autodiff::real, N * n_controls + (N + 1) * n_states, 1 > & vars,
    ...
    func_Jacobian = autodiff::jacobian([ & ](auto x) {
      return multiple_shooting_objective(x);
    }, wrt(vars), at(vars), res);
    result = res(0, 0).val();
  }
  ...
  void MPC::get_constraints_jacobian(Eigen::Matrix < autodiff::real, N * n_controls + (N + 1) * n_states, 1 > vars,
    ...
    constr_Jacobian = autodiff::jacobian([ & ](auto x) {
      return multiple_constraints(x);
    }, wrt(vars), at(vars), res);
    //std::cout<<"\nStolbcov: "<<Jx.cols()<<"\nRiadov:"<<Jx.rows()<<"\n";
  ...
  void MPC::set_reference(const Eigen::Matrix < autodiff::real, n_states, N > & ref_path,
  ...
  void MPC::multi_constraint(unsigned m, double * result, unsigned n,
